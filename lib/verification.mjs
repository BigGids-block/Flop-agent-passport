...
function newest(records) { return records.reduce((latest, record) => !latest || record.sequence > latest.sequence ? record : latest, null); }

export async function verifyContributor(did, archive = null) {
  if (!DID_PATTERN.test(did)) throw new Error('A valid did:key is required.');
  const cacheKey = `did|${did}`;
  const cached = cache.get(cacheKey);
  if (cached && Date.now() - cached.created < CACHE_MS) return cached.data;
  if (archive) {
    const archived = (await archive.findByDid(did)).filter((record) => record && record.did === did && record.nonce != null && urlsIn(record.text || '').length);
    const match = newest(archived);
    if (match) {
      const data = { verified: true, lookup: 'RECORD FOUND', record: publicRecord(match, 'durable contribution archive') };
      cache.set(cacheKey, { created: Date.now(), data });
      return data;
    }
  }
  let room;
  try { room = await recentRecords(); } catch { return { verified: false, lookup: 'CHECK UNAVAILABLE', summary: 'The live Technocore record check is temporarily unavailable. Please try again.' }; }
  const matches = room.messages.filter((record) => record.did === did && record.nonce != null && urlsIn(record.text || '').length);
  const match = newest(matches);
  ...
  console.info('technocore DID lookup', JSON.stringify({ ...diagnostics, did, matchedSequence: match?.sequence ?? null }));
  if (match && archive) { try { await archive.put(match); } catch (error) { console.error('technocore archive write failed', error); } }
  const data = match ? { verified: true, lookup: 'RECORD FOUND', record: publicRecord(match, 'live Technocore feed'), diagnostics } : { verified: false, lookup: 'NO RECORD FOUND', summary: 'No signed public contribution was found for this DID.', diagnostics };
  cache.set(cacheKey, { created: Date.now(), data });
  return data;
}
